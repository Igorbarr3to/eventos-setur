...
export function GerenciadorDePerguntasTemplate({ templateInicial }: GerenciadorProps) {
  ...
  const refetchPerguntas = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Falha ao buscar perguntas do template.");
  ...
  return (
    <div className="space-y-4 border-t pt-6">
      <div className="flex justify-between items-center">
        ...
        <CriarPerguntaTemplateModal templateId={template.id} onPerguntaCriada={refetchPerguntas} />
      </div>
        ...
        <div className="border rounded-lg bg-white">
          <ul className="divide-y">
            {perguntas.map((pergunta) => (
              <li key={pergunta.id} className="flex justify-between items-center p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <EditarPerguntaTemplateModal pergunta={pergunta} onPerguntaEditada={refetchPerguntas} />
                  <DeletarPerguntaTemplateBotao perguntaId={pergunta.id} onPerguntaDeleted={refetchPerguntas} />
                </div>
              </li>
            ))}
          </ul>
