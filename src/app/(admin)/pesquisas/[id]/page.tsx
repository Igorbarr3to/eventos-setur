...
async function getPesquisaById(id: string): Promise<Pesquisa | null> {
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/pesquisas/${id}`, {
...
export default async function PaginaDetalhePesquisa({ params }: { params: { id: string } }) {
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8 space-y-8">
            ...
            <div className="flex justify-between items-start">
                ...
                <Badge variant="secondary" className="text-sm">{pesquisa.status}</Badge>
            </div>

            {/* Seção de Detalhes do Projeto */}
            {pesquisa.tipo === 'EVENTO' && (
                <div className="space-y-4 p-6 border rounded-lg bg-slate-50">
                    <h2 className="text-xl font-semibold border-b pb-2">Detalhes do Projeto</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                        <div><strong>Título do Projeto:</strong> {pesquisa.tituloProjeto || 'N/A'}</div>
                        <div><strong>Proponente:</strong> {pesquisa.proponente || 'N/A'}</div>
                        <div><strong>Município:</strong> {pesquisa.municipio || 'N/A'}</div>
                        <div><strong>Local:</strong> {pesquisa.localAplicacao || 'N/A'}</div>
                        <div><strong>Início:</strong> {new Date(pesquisa.dataInicio!).toLocaleDateString('pt-BR')}</div>
                        <div><strong>Fim:</strong> {new Date(pesquisa.dataFim!).toLocaleDateString('pt-BR')}</div>
                        <div className="col-span-2"><strong>Objetivo Geral:</strong> {pesquisa.objetivoGeral || 'N/A'}</div>
                    </div>
                </div>
            )}
